...
void ihm_format_lines(float position_mm, float velocity_mm_s, machine_state_t state, char *line1, char *line2) {
    ...
    if (isnan(position_mm) || isinf(position_mm)) position_mm = 0.0f;
    if (isnan(velocity_mm_s) || isinf(velocity_mm_s)) velocity_mm_s = 0.0f;
...
static esp_err_t pcf8574_write_byte(ihm_display_t *ihm, uint8_t data) {
    if (!ihm || !ihm->i2c_dev_handle) return ESP_FAIL;
...
void ihm_init(ihm_display_t *ihm, const ihm_config_t *config) {
    ...
    if (i2c_new_master_bus(&bus_config, &bus_handle) == ESP_OK) {
        ...
        if (i2c_master_bus_add_device(bus_handle, &dev_config, &dev_handle) == ESP_OK) {
            ...
            ESP_LOGI(TAG, "IHM LCD I2C connected successfully at 0x%02X", ihm->config.i2c_address);
        }
    }
#else
    ihm->is_connected = true;
...
bool ihm_write_lcd(ihm_display_t *ihm, const char *line1, const char *line2) {
    if (!ihm || !line1 || !line2) return false;
